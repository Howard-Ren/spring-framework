...
class ConfigurationClassParser {
	...
	protected final SourceClass doProcessConfigurationClass(
		...
		//1、内部类的递归解析
		//只对@Configuration和@Component标注的类进行内部类的解析
		//@ComponentScan,@Import,@ImportSource不解析内部类
		...
		//2、@PropertySource注解的解析
		// 通过@PropertySource注解引入的property，只能是.properties格式，如果是yml格式，需要配置一个PropertySourceFactory这个属性
		// 解析的时候可以借助Spring的YamlPropertiesFactoryBean类
		...
		//3、处理@ComponentScan注解，注意，在这个步骤中扫描到的会直接注册到Registry中
			...
			for (AnnotationAttributes componentScan : componentScans) {
				...
				for (BeanDefinitionHolder holder : scannedBeanDefinitions) {
					...
					if (ConfigurationClassUtils.checkConfigurationClassCandidate(bdCand, this.metadataReaderFactory)) {
						//如果扫描到的BeadDefinition也是个配置类，则继续递归解析。
		...
		if (importResource != null) {
			...
			for (String resource : resources) {
				...
				//保存到当前ConfigurationClass的importedResources属性中
		...
		// 处理该配置类实现的接口中被@Bean注解标注的Default方法
				...
				for (SourceClass candidate : importCandidates) {
					if (candidate.isAssignable(ImportSelector.class)) {
						...
						if (selector instanceof DeferredImportSelector) {
							//如果实现了DeferredImportSelector接口，则保存到当前配置类ConfigurationClass的deferredImportSelectors属性中
							this.deferredImportSelectorHandler.handle(configClass, (DeferredImportSelector) selector);
						} else {
							//否则，则把ImportSelector接口中selectImports方法返回的所有类当做一个被@Import注解引入的类，递归调用本方法
							String[] importClassNames = selector.selectImports(currentSourceClass.getMetadata());
							Collection<SourceClass> importSourceClasses = asSourceClasses(importClassNames, exclusionFilter);
							processImports(configClass, currentSourceClass, importSourceClasses, exclusionFilter, false);
						}
					} else if (candidate.isAssignable(ImportBeanDefinitionRegistrar.class)) {
						//如果导入的类，实现了ImportBeanDefinitionRegistrar接口
						...
						ImportBeanDefinitionRegistrar registrar =
								ParserStrategyUtils.instantiateClass(candidateClass, ImportBeanDefinitionRegistrar.class,
										this.environment, this.resourceLoader, this.registry);
						//放入当前配置类ConfigurationClass的ImportBeanDefinitionRegistrar属性中
						configClass.addImportBeanDefinitionRegistrar(registrar, currentSourceClass.getMetadata());
					} else {
						// Candidate class not an ImportSelector or ImportBeanDefinitionRegistrar ->
						// process it as an @Configuration class
						//最后，如果导入的类，没有实现上面2种接口，则当做一个默认的配置类来处理
